...
>
> user에 관련된 데이터를 table에서 전부 검색해서 보여줍니다.
>
> 예상결과 : 
>
> ```
...
> 일단 첫번째 과제에서 방문객들의 정보를 기존 환자와 일일이 대조하는 부분이 있는데, 이부분은 새로운 테이블을 만들어서 저장하는것도 좋다고 생각한다. 물론 데이터의 중복이 일어나고, 저장시에 조금 더 시간이 걸리지만, 속도면에서 배는 빠르게 진행이 된다고 생각한다.
>
> 두번째로 테이블간에 결합이 전혀 없어서 결합을 해주는것이 좋다 생각한다. person_id같은 경우는 중복이 많아서 외래키로 지정만 해도, 역참조등으로 테이블 사이에서 연산이 매우 빨라질것이다.
>
> 정규화가 매우매우 필요하다.
>
